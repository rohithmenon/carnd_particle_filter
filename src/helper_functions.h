...
struct LandmarkObs {
	...
	LandmarkObs(int id_val, double x_val, double y_val): id(id_val), x(x_val), y(y_val) {}

	bool operator<(const LandmarkObs& ops) const {
		if (id == ops.id) {
      if (x == ops.x) {
        return y < ops.y;
      } else {
	...
	friend std::ostream &operator<<(std::ostream &os, const LandmarkObs& obs) {
		return os << "[" << obs.id << "," << obs.x << "," << obs.y << "]";
	}
};

typedef std::vector<LandmarkObs> Map;
...
inline bool read_map_data(std::string filename, Map& map) {
	...
	while(getline(in_file_map, line_map)){
		...
		LandmarkObs single_landmark_temp{id_i, landmark_x_f, landmark_y_f};

		// Add to landmark list of map:
		map.emplace_back(single_landmark_temp);
	}
	return true;
}
...
inline bool read_landmark_data(std::string filename, std::vector<LandmarkObs>& observations) {
	...
	while(getline(in_file_obs, line_obs)){
		...
		LandmarkObs meas{-1, local_x, local_y};

		// Add to list of control measurements:
		observations.emplace_back(meas);
	}
	return true;
}
